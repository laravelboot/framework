...
use LaravelBoot\Foundation\ServiceManager\Exception\ServerConfigException;
use LaravelBoot\Foundation\Network\Connection\NovaClientConnectionManager;

class ServiceDiscoveryInitiator
{
    use Singleton;
    ...
    public function init($workerId)
    {
        ...
        foreach ($config['app_names'] as $appName) {
            ...
            $appConfigs[$appName] += [
                "protocol" => ServiceDiscovery::DEFAULT_PROTOCOL,
                "namespace" => ServiceDiscovery::DEFAULT_NAMESPACE
            ];
        }
        ...
        // if (ServerStore::getInstance()->lockDiscovery($workerId)) {
            ...
            foreach ($config['app_names'] as $appName) {
                ...
                $serverDiscovery = new ServiceDiscovery($config, $appName, $appConf["protocol"], $appConf["namespace"]);
    ...
    public function noNeedDiscovery($config)
    {
        ...
        foreach ($noNeedDiscovery['app_names'] as $appName) {
            ...
            $novaConfig += [ "domain" => ServiceDiscovery::DEFAULT_NAMESPACE ];
